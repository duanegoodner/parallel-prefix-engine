...
__device__ void ComputeAndStoreTilePrefixSum(
    ...
    tile_smem[index_2d(local_x, local_y, tile_width)] = params.array.d_address[global_idx];
    __syncthreads();

    // === Phase 2: Local 2D prefix sum ===
    ComputeTilePrefixSumInShared(tile_smem, tile_height, tile_width, local_x, local_y);
    __syncthreads();

    // === Phase 3: Write result back ===
    params.array.d_address[global_idx] = tile_smem[index_2d(local_x, local_y, tile_width)];
    __syncthreads();

    // === Phase 4: Collect right edge ===
    if (tile_j < grid_dim_y - 1 && local_y == tile_width - 1) {
        int edge_idx = global_x * (grid_dim_y - 1) + tile_j;
        right_edges[edge_idx] = tile_smem[index_2d(local_x, tile_width - 1, tile_width)];
    }

    // === Phase 5: Collect bottom edge ===
    if (tile_i < grid_dim_x - 1 && local_x == tile_height - 1) {
        int edge_idx = tile_i * array_width + global_y;
        bottom_edges[edge_idx] = tile_smem[index_2d(tile_height - 1, local_y, tile_width)];
    }
}
